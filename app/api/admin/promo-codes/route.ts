...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ message: "Unable to fetch promo codes" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { code, type, value, maxUses, minBookingAmount, maxDiscountAmount,
            validFrom, validUntil, title, imageUrl } = body;
    ...
    if (!["percentage", "fixed", "free_ticket"].includes(type)) {
      return NextResponse.json({ message: "Invalid promo code type" }, { status: 400 });
    }
    if (!validFrom || !validUntil) {
      return NextResponse.json({ message: "Valid from and valid until dates are required" }, { status: 400 });
    }
    ...
    return NextResponse.json({ message: "Promo code created", promoCode }, { status: 201 });
  } catch (error) {
    console.error("Error creating promo code:", error);
    return NextResponse.json({ message: "Unable to create promo code" }, { status: 500 });
  }
}
