...
export default function SearchPageClient({
  ...
  const [waitlistBus, setWaitlistBus] = useState<BusSummary | null>(null);
  const [joinedWaitlistIds, setJoinedWaitlistIds] = useState<Set<string>>(new Set());
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
        ...
        <div className="space-y-4">
          {sortedBuses.length === 0 ? (
            ...
            sortedBuses.map((bus) => {
              ...
              const hasJoined = joinedWaitlistIds.has(bus.id);
              ...
              return (
                <Card
                  ...
                  onClick={() => !isFull && router.push(bookingHref)}
                >
                  ...
                  <div className="grid grid-cols-3 gap-4 px-6 text-sm">
                    <div>
                  ...
                  <CardContent className="space-y-4 pt-4">
                    ...
                    {isFull ? (
                      hasJoined ? (
                        <div className="flex h-11 w-full items-center justify-center rounded-xl border border-amber-200 bg-amber-50 text-sm font-medium text-amber-700">
                          ✓ On the waitlist
                        </div>
                      ) : (
                        <Button
                          className="w-full h-11 rounded-xl border-amber-300 bg-amber-50 text-amber-800 hover:bg-amber-100 shadow-sm"
                          variant="outline"
                          onClick={(e) => {
                            e.stopPropagation();
                            setWaitlistBus(bus);
      ...
      {waitlistBus && (
        <WaitlistDialog
          open={!!waitlistBus}
          onOpenChange={(open) => !open && setWaitlistBus(null)}
          bus={waitlistBus}
          onJoined={() => {
            setJoinedWaitlistIds((prev) => new Set([...prev, waitlistBus.id]));
            setWaitlistBus(null);
          }}
        />
      )}
